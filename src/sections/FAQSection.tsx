...
export function FAQSection() {
  ...
  return (
    <section className="section-y bg-fog/50 border-t border-ink-100/50 dark:border-ink-800/40">
          ...
          {homeFaqs.map((faq, index) => {
            ...
            return (
              <motion.div
                ...
                className="glass rounded-apple-xl border border-ink-100/60 dark:border-ink-700/30 overflow-hidden transition-all duration-fast"
              >
                <button
                ...
                >
                  <span className="font-semibold text-[16px] sm:text-[18px] text-foreground tracking-tight flex items-center gap-3">
                    <HelpCircle className="w-5 h-5 text-sky shrink-0" strokeWidth={1.75} />
                    {faq.question}
                  </span>
                  <ChevronDown
                    className={cn(
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="px-5 sm:px-6 pb-6 pt-1 text-[15px] sm:text-[16px] text-ink-500 leading-snug2 border-t border-ink-100/40 dark:border-ink-800/40">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
