  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          <p className="text-sky text-[12px] font-semibold uppercase tracking-[0.12em] mb-1">
            360° Tours
          </p>
          <h1 className="text-xl md:text-2xl font-semibold text-foreground flex items-center gap-2">
            <Box className="w-6 h-6 text-sky" strokeWidth={1.5} />
            Architecture design
          </h1>
        </div>
        <Link href="/admin/architecture-design/new">
          <Button>
        ...
        <div className="space-y-3">
          {filtered.map((item, i) => (
            <motion.div
              ...
              className="flex gap-4 p-4 bg-card border border-ink-100/60 dark:border-ink-700/40 rounded-2xl"
            >
              <div className="relative w-24 h-16 sm:w-32 sm:h-20 rounded-xl overflow-hidden bg-fog shrink-0">
                {item.coverImage ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-sky/10 text-sky border border-sky/20">
                    {item.category}
                  </span>
                  {item.isFeatured && (
                    <Star className="w-3.5 h-3.5 text-bhutan-gold fill-bhutan-gold" />
                  )}
                  {item.isPinned && <Pin className="w-3.5 h-3.5 text-ink-400" />}
                </div>
                <p className="text-[13px] text-ink-500 line-clamp-1">{item.summary}</p>
